/- 17 - Using Our Own Lemma
-/

import Mathlib.Tactic


lemma Nat.le_or_succ_le (a b: ℕ) : a ≤ b ∨ b + 1 ≤ a := by
    rw [Nat.add_one_le_iff]
    exact le_or_gt a b

/-
* For any natural number n, show that  n^3 ≠ 10

Proof strategy - split the number sequence into tow

    all the smaller number result in n^3 < 10

    all the larger numbers result in n^3 > 10

    cubes: 0:0 1:1 2:8 3:27 4:64 so:  (2,3)

If every possible choice for n leads to either n^2 < 7 ∨ n^2 > 7
then n^2 ≠ 7

Solution:
    n^2 ≠ 10            proof objective

    n ≤ 2 ∨ 3 ≤ n       lemma (1) with m = 2

    case n ≤ 2          using (2)
        n^3 ≤ 8
            < 10
        n^3 ≠ 10         lenna a < b → a ≠ b

    case n ≥ 3
        n^3 ≥ 9
            > 10
        n^3 ≠ 10        lemma a > b → a ≠ b

        n ≠ 10           conclusion of both cases
-/

example {n : ℕ} : n^3 ≠ 10  := by
    have h := Nat.le_or_succ_le n 2
    obtain ha | hb := h
    · apply ne_of_lt
      calc
        n^3 ≤ 2^3 := by rel [ha]
        _ < 10 := by norm_num
    · apply ne_of_gt
      calc
        n^3 ≥ 3^3 := by rel [hb]
        _ > 10 := by norm_num
